...
export default function ChatComponent({
  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto">
      ...
      <div className="bg-white rounded-xl border border-[#E2E8F0] overflow-hidden shadow-sm flex flex-col h-[480px]">
        
        {/* Chat Control Bar */}
        <div className="bg-[#F8FAFC] px-4 py-2.5 border-b border-[#E2E8F0] flex items-center justify-between">
          <div className="flex items-center gap-2">
            <span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse" />
            <span className="text-[10px] font-black uppercase text-[#0F172A]">Sala de Passagem Geral</span>
          </div>

          <div className="flex items-center gap-3">
            <div className="text-[#64748B] text-[10px] font-bold">
              Enviando como: <span className="text-[#0F172A] font-extrabold">{currentLeader.name}</span>
            </div>
            ...
            {isAdmin && (
              <button
                ...
                className="text-[#2563EB] hover:underline p-1 rounded transition-colors text-[10px] font-black flex items-center gap-1 cursor-pointer"
                title="Limpar Chat"
              >
        ...
        {/* Message stream */}
        <div className="flex-1 overflow-y-auto p-4 bg-[#F8FAFC]/50 space-y-3">
          {messages.length === 0 ? (
              ...
              return (
                ...
                >
                  ...
                  <div className="flex items-center gap-1.5 mb-0.5 px-1">
                    <span className="text-[10px] font-black text-[#0F172A]">{msg.senderName}</span>
                    <span className="text-[9px] text-[#64748B]">({msg.senderRole})</span>
                  </div>

                  {/* Bubble body */}
                  <div 
                    className={`rounded-xl px-3 py-2 text-xs font-medium leading-relaxed shadow-sm border ${
                      isMe 
                        ? 'bg-[#2563EB] text-white border-[#1D4ED8] rounded-tr-none' 
                        : 'bg-white text-[#0F172A] border-[#E2E8F0] rounded-tl-none'
                    }`}
                  >
                  ...
                  {/* Timestamp */}
                  <span className="text-[9px] text-[#64748B] mt-0.5 px-1">{formatMessageTime(msg.timestamp)}</span>
                </div>
              );
            })
          )}
        ...
        <div className="bg-white p-3 border-t border-[#E2E8F0]">
          <form onSubmit={handleSend} className="flex gap-2">
            <input
              ...
              className="flex-1 bg-[#F8FAFC] border border-[#E2E8F0] rounded-lg px-3 py-2 text-xs text-[#0F172A] focus:outline-none focus:ring-1 focus:ring-[#2563EB] font-medium"
              required
            />
            <button
              type="submit"
              className="p-2.5 bg-[#2563EB] text-white rounded-lg hover:bg-[#1D4ED8] transition-colors shadow-sm flex items-center justify-center cursor-pointer"
            >
              <Send className="w-3.5 h-3.5" />
            </button>
          </form>
          <div className="flex items-center gap-1 mt-1.5 text-[9px] text-[#64748B] font-medium">
